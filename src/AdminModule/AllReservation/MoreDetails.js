...
const OtaAuditCallout = styled.div`
	...
	button {
		...
		border: 1px solid #2f6db3 !important;
...
const ReservationDetail = ({
	...
	const canLimitedOrderTakerEditReservation =
		canFullManageReservation || isOrderTakerEditableReservation(reservation);
	...
	const handleOpenOtaEmailAudit = async () => {
		...
		try {
			...
			if (!data || data.error || data.ok === false) {
				toast.error(data?.error || data?.message || "Could not load inbound email audit.");
		...
		(reservation?.pickedRoomsType || []).forEach((room, roomIndex) => {
			...
			const dayRows = hasDailyPricing
				? pricingRows.map((day, dayIndex) => {
						const clientPrice = normalizeNumber(
							day?.clientPrice ??
								day?.mainPrice ??
								day?.totalPriceWithCommission ??
								day?.price ??
					...
					>
						{reservation && (
							<EditReservationMain
								...
								onReservationUpdated={handleEditReservationSaved}
							/>
						)}
					</Modal>
							...
							<div>
								<EmailAuditMeta>
									...
									<div>
										<strong>Total</strong>
										<br />
										{normalizeNumber(otaEmailAudit.totalAmountSar, 0).toFixed(
											2,
										)}{" "}
										SAR
						...
						>
							<div className='pricing-summary-row'>
								...
								<div className='pricing-summary-card'>
									...
									<strong className='detail-value-ltr'>
										{formatMoney(pricingBreakdownByDay.rootTotal)}{" "}
										{chosenLanguage === "Arabic" ? AR_LABELS.currency : "SAR"}
									</strong>
								</div>
								<div className='pricing-summary-card'>
							...
							<Section className='top-request-card'>
								...
								{canFullManageReservation ? (
									<div className='top-document-actions'>
										...
										<button
											className='top-aldawleya-button'
								...
								<div className='middle-operation-shell'>
									...
									{canViewOtaEmailAudit && createdByOtaEmail ? (
										<OtaAuditCallout>
											...
											<div>Source: {String(otaAutomationLabel || "OTA email")}</div>
											...
											{otaInboundEmailId ? (
												<button type='button' onClick={handleOpenOtaEmailAudit}>
													View inbound email
												</button>
											) : (
