...
const MoreDetails = ({
  ...
  }, [hotelDetails, reservation, user?._id]);
  ...
  const rawHotelIdValue =
    reservation?.hotelId || hotelDetails?._id || hotelDetails;
      ...
      (Array.isArray(rows) ? rows : [])
        ...
        .map((room) => {
          const roomType = String(
            room?.room_type || room?.roomType || room?.label || "",
          ).trim();
          const roomNumber = String(
            room?.room_number || room?.roomNumber || room?.number || "",
          ).trim();
          return {
            room_type: roomType,
        ...
        .filter((room) => room.room_type || room.room_number);

    const inferredTypeFromReservation = (() => {
      const fromPicked = Array.isArray(reservation?.pickedRoomsType)
        ? Array.from(
            new Set(
              reservation.pickedRoomsType
                .map((item) =>
                  String(item?.room_type || item?.roomType || "").trim(),
                )
                .filter(Boolean),
            ),
          ).join(" + ")
      ...
      const fromBreakdown = Array.isArray(reservation?.roomBreakdown)
        ? reservation.roomBreakdown
            .map((item) => String(item?.label || item?.room_type || "").trim())
            .filter(Boolean)
            .join(" + ")
        : "";
      return fromBreakdown || "Room";
    })();

    const fromDetails = normalizeRows(
      Array.isArray(reservation?.roomDetails) ? reservation.roomDetails : [],
    );
    if (fromDetails.length > 0) return fromDetails;

    const fromChosen = normalizeRows(chosenRooms);
    if (fromChosen.length > 0) return fromChosen;

    const fromRoomId = normalizeRows(
      Array.isArray(reservation?.roomId)
        ? reservation.roomId.filter((room) => room && typeof room === "object")
        : [],
    );
    if (fromRoomId.length > 0) return fromRoomId;

    const fromRoomNumbers = normalizeRows(
      Array.isArray(reservation?.room_numbers)
        ? reservation.room_numbers.map((entry) => {
            if (entry && typeof entry === "object") return entry;
            return {
              room_number: entry,
              room_type: inferredTypeFromReservation,
            };
          })
        : [],
    );
    if (fromRoomNumbers.length > 0) return fromRoomNumbers;

    const fromBedNumbers = normalizeRows(
      Array.isArray(reservation?.bedNumber)
        ? reservation.bedNumber.map((entry) => ({
            room_number: entry,
            room_type: "Bed",
          }))
        : [],
    );
    if (fromBedNumbers.length > 0) return fromBedNumbers;

    const fromPickedRoomsType = normalizeRows(
      Array.isArray(reservation?.pickedRoomsType)
        ? reservation.pickedRoomsType.map((entry) => {
            const count = Number(entry?.count || 0);
            const label = String(
              entry?.room_type || entry?.roomType || "Room",
            ).trim();
            return {
              room_type: count > 1 ? `${label} (x${count})` : label,
              room_number: "",
            };
          })
        : [],
    );
    if (fromPickedRoomsType.length > 0) return fromPickedRoomsType;

    const fromBreakdown = normalizeRows(
      Array.isArray(reservation?.roomBreakdown)
        ? reservation.roomBreakdown.map((entry) => {
            const count = Number(entry?.count || 0);
            const label = String(
              entry?.label || entry?.room_type || "Room",
            ).trim();
            return {
              room_type: count > 1 ? `${label} (x${count})` : label,
              room_number: "",
      ...
      {loading ? (
        ...
        <div className="otherContentWrapper">
          ...
          <Modal
            ...
            centered
            zIndex={50050}
            styles={{ mask: { zIndex: 50049 } }}
            getContainer={() => document.body}
            wrapClassName="payment-breakdown-modal"
            rootClassName="payment-breakdown-modal"
            destroyOnClose
          >
